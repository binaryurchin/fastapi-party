...
def test_party_detail_form_partial_updates_party_and_returns_updated_party(
    ...
    assert response.status_code == status.HTTP_200_OK
    assert (
        party.party_date == datetime.strptime(updated_data["party_date"], "%Y-%m-%d").date()
    )
    assert (
        party.party_time == datetime.strptime(updated_data["party_time"], "%H:%M").time()
    )
    assert (
        party.invitation == updated_data["invitation"]
    )
    assert (
        party.venue == updated_data["venue"]
    )