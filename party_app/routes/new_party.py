...
router = APIRouter(prefix="/party/new", tags=["new-party"])

@router.get("/", response_class=HTMLResponse, name="new_party_form_page")
...
@router.post("/", response_class=HTMLResponse, name="new_party_create_page")
def new_party_create_page(
    ...
    return RedirectResponse(request.url_for("party_detail_page", party_id=party.uuid),
...
def validate_date(
    ...
    if party_date < date.today():
        return HTMLResponse(
            content="You chose a date in the past",
            status_code=status.HTTP_200_OK,
        )

    return Response(status_code=status.HTTP_204_NO_CONTENT)

@router.post("/validate_invitation", name="validate_invitation_partial", response_class=Response)
